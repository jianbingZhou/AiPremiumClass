...
class Encoder(nn.Module):
    def __init__(self, input_dim, emb_dim, hidden_dim, state_type='concat'):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_out):
         # a_t = h_t @ h_s  
        a_t = torch.bmm(encoder_out, decoder_out.permute(0, 2, 1))
        # 1.计算 结合解码token和编码token，关联的权重
        a_t = torch.softmax(a_t, dim=1)
        # 2.计算 关联权重和编码token 贡献值
        c_t = torch.bmm(a_t.permute(0, 2, 1), encoder_out)
        return c_t    
...
if __name__ == '__main__':
    ...
    logits, hidden = seq2seq(torch.randint(0, input_dim, (batch_size, seq_len)),
                             torch.randint(0, input_dim, (batch_size, seq_len)))
    print(logits.shape)  # [batch_size, seq_len, input_dim]